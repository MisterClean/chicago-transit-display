...
export const scheduleSchema = z.object({ version: label, imported_at: timestamp, coverage_start: z.iso.date(), coverage_end: z.iso.date(), valid_until: timestamp });
export const providerSchema = z.object({ realtime_configured: z.boolean().optional(), active_source: z.enum(['realtime', 'schedule', 'none']).optional(), schedule: scheduleSchema.optional(), id, name: label, connection_state: z.enum(['enabled', 'pending', 'unavailable', 'disabled']), message: z.string().max(2000).optional(), attribution: z.string().max(2000), terms_url: z.string().url().startsWith('https://') });
