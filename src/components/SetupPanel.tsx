...
export default function SetupPanel({ config, setConfig, catalog, providers, mode, setMode, onClose, onDisplay }: Props) {
  ...
  return <div className="setup-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <div ref={panelRef} className="setup-panel" role="dialog" aria-modal="true" aria-labelledby="setup-title">
      <header className="setup-header"><div><span className="dialog-eyebrow">MAKE YOURSELF AT HOME</span><h2 id="setup-title">Your neighborhood, your way.</h2><p>Choose what’s useful. We’ll keep it close.</p></div><button ref={closeRef} className="icon-button close-setup" aria-label="Close customization" onClick={onClose}><X size={22} /></button></header>
...
function LocationSettings({ config, setConfig, catalog, announce }: SettingsProps & { catalog: Catalog; announce: Announce }) {
  ...
  return <div className="settings-stack">
    <div className="settings-intro"><h3>Where’s your front door?</h3><p>Set one entrance for distances and nearby vehicles. Move the pin until it feels right.</p></div>
...
function ConnectionsSettings({ config, setConfig, catalog, providers, mode, announce }: SettingsProps & { catalog: Catalog; providers: Provider[]; mode: 'demo' | 'live'; announce: Announce }) {
  ...
  return <div className="settings-stack">
    <div className="settings-intro connections-intro"><div><h3>A few good connections.</h3><p>Pin the places and rides you reach for most.</p></div><span className="capacity-pill">{total}<span>/ 12 cards</span></span></div>
    ...
    </> : <div className="pinned-list">{!config.selections.length && !config.vehicle_rules.length && <div className="no-results"><MapPin size={26} /><strong>Your board is a blank canvas.</strong><p>Find a nearby stop to add your first connection.</p></div>}{config.selections.map((selection, index) => {
...
function DisplaySettings({ config, setConfig, mode, setMode }: SettingsProps & Pick<Props, 'mode' | 'setMode'>) {
  ...
  return <div className="settings-stack">
    <div className="settings-intro"><h3>A board that belongs here.</h3><p>A few small adjustments make it yours.</p></div>
...
function SaveSettings({ config, setConfig, mode, announce }: SettingsProps & Pick<Props, 'mode'> & { announce: Announce }) {
  ...
  return <div className="settings-stack">
    <div className="settings-intro"><h3>Take your neighborhood with you.</h3><p>No sign-in, no account. Keep a backup or bring this board to another screen.</p></div>
