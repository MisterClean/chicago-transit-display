...
function DisplaySettings({ config, setConfig, mode, setMode }: SettingsProps & Pick<Props, 'mode' | 'setMode'>) {
  ...
  return <div className="settings-stack">
    ...
    <div className="setting-row"><div><strong>Board data</strong><span>Choose sample data or real schedules and connected providers.</span></div><div className="segmented-control"><button className={mode === 'demo' ? 'active' : ''} aria-pressed={mode === 'demo'} onClick={() => setMode('demo')}>Demo</button><button className={mode === 'live' ? 'active' : ''} aria-pressed={mode === 'live'} onClick={() => setMode('live')}>Live</button></div></div>
