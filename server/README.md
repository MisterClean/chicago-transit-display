...
Realtime transit and station observations become stale after 90 seconds and expire after 180 seconds, using the older of record and envelope timestamps. Individual vehicles are selectable only for 90 seconds and positions are removed from memory by the 120-second expiry policy/30-second cleanup tick. Requests also run expiry cleanup. Provider failures retain only observations inside these limits and return partial boards. Expired fixed observations keep status timestamps but erase event/count data. Source clocks more than 30 seconds ahead are rejected. CTA timezone-less dates use America/Chicago; invalid spring-transition local times fail, and ambiguous fall-transition source times use the nearest UTC candidate to the request time; arrival candidates must follow their source observation when possible.

## Feed verification and limits

The [all-provider launch review](../docs/mobility-compliance-review.md) records current blockers and required evidence. The [CTA developer review](../docs/cta-compliance-review.md) covers adapter details. Bus v3 dynamic action flags preserve cancellations and pickup-skipped service, hide invalidated trips, and reject unrecognized actions. Mixed scheduled/live train times are labeled individually in the browser.

The CTA adapters follow the official [Bus Tracker documentation](https://www.transitchicago.com/developers/bustracker/) and [Train Tracker reference](https://www.transitchicago.com/developers/ttdocs/). Bus `unixTime=true` timestamps are milliseconds. Train schedule, approaching, delay and fault flags are preserved; faulted events do not present dependable countdowns. Events preserve arrivals versus departures only where supplied explicitly (bus); Train Tracker records are labeled arrivals. **Authenticated CTA requests have not been validated with operator-issued keys in this build.** Published GTFS schedules are used only when realtime credentials are absent. No CTA disruption-alert collector is included; an empty prediction list does not establish that service has stopped. Published overview defaults are 100,000 bus and 50,000 train requests/day; the issued account limit governs. The implementation uses lower fixed concurrency/demand limits, not a claimed contractual quota.

The [Divvy 2.3 discovery feed](https://gbfs.divvybikes.com/gbfs/2.3/gbfs.json), `system_information`, `vehicle_types`, `station_information`, `station_status` and `free_bike_status` were retrieved and inspected September 14, 2026. Observed types included human-powered bicycles, electric-assist bicycles and electric scooters. The metadata identifies `lyft_chi` / America/Chicago and currently omits `license_url`. The [Divvy system-data page](https://divvybikes.com/system-data) and [published data license](https://divvybikes.com/data-license-agreement) are the review references. Before public launch, establish the applicable live-feed/public-display license and required written permission for the Divvy name/marks. An enablement setting is not permission evidence; public launch remains blocked on this review. Lime and Spin are disabled; this is not complete scooter-operator coverage.

Metra stations come from its [official schedule archive](https://schedules.metrarail.com/gtfs/schedule.zip). The schedule importer supplies real published departures without realtime credentials. During the first schedule download, Metra shows a loading state. Metra realtime requires a separate adapter.

Keep Geocodio disabled for a public launch until account-use eligibility, privacy conditions and underlying-source attribution are resolved; see the launch review. [Geocodio's current API reference](https://www.geocod.io/docs/) specifies the v2 endpoint. Submitted address lookup is limited to one in-flight request and one submission every two seconds across the process. There is no persistent address cache or query logging. Add appropriate authenticated edge limits for a publicly exposed deployment; the process-wide limits are not user authentication.
...
## Published schedules in Live mode

The server automatically downloads official CTA and Metra GTFS feeds in the background on every startup, then every 24 hours. No API keys or manual import are needed. Start normally:

```sh
cargo run --release --manifest-path server/Cargo.toml --bin near-and-next-server
```

A valid local cache is loaded immediately. Without one, the server still starts and transit cards display “Downloading published schedules” until the first import succeeds. A corrupt cache is rebuilt. Downloads, CSV parsing, validation and disk writes run outside board requests; CPU-heavy work uses a blocking worker. Successful refreshes replace the on-disk cache and activate the new schedule/catalog together in memory. Existing realtime snapshots, demand and Divvy station metadata are preserved.

The generated `server/data/runtime/schedules.json` and downloaded timetable data are not committed to Git. `SCHEDULE_PATH` can point to a writable persistent-cache location; its parent directories are created automatically. Both operator archives must pass validation, including current service coverage, before the cache is replaced. A failed download, invalid feed or failed write retains the previous cache and running schedule, with retry after 15 minutes. Expired cached schedules remain unavailable rather than being extended. A first-run failure is reported on the board and retried automatically.

Allow up to three minutes per download. CTA's September 2026 stop-times table is approximately 367 MB uncompressed; the combined index is approximately 118 MB on disk. The importer streams CSV rows. Allow several hundred MB of memory, including room for the previous index during refresh, and measure the host before sizing it. One schedule-refresh task runs per server process; use a single process per cache path.

An optional CLI supports manual refresh or reproducible local-archive debugging:

```sh
cargo run --release --manifest-path server/Cargo.toml --bin import-schedules
cargo run --release --manifest-path server/Cargo.toml --bin import-schedules -- /path/schedules.json /path/cta.zip /path/metra.zip
SCHEDULE_PATH=/path/schedules.json cargo run --release --manifest-path server/Cargo.toml --bin near-and-next-server
```

Manual imports validate references, calendars, time ranges, archive/table size limits and minimum mapped coverage, then atomically replace the artifact. Restart to load a manual import immediately; background updates activate without a restart. The server always attempts a fresh official download on startup, including when a custom cache path is configured.

Source selection is per provider: missing CTA Bus/Train Tracker credentials use schedules; configured credentials always use the existing realtime collector, including cold start, empty responses and outages. Adding the corresponding key and restarting activates predictions. Metra uses schedules until a realtime adapter is implemented. Divvy remains independent GBFS availability. These CTA adapters are JSON Tracker APIs, not GTFS-Realtime adapters.

Queries use a three-hour departure window, with the existing per-route/destination limits and 100-event cap. Saved place IDs stay unchanged. CTA platform stops map to parent stations; routes enrich catalog selections. Stops without a validated mapping report unavailable. Calendars include weekday rules and additions/removals; previous service days and times through 71:59:59 use GTFS noon-minus-twelve-hours semantics in America/Chicago. Drop-off-only and conditional pickups are omitted. Exact frequency service (`exact_times=1`) is expanded; non-exact frequency service and missing stop times are omitted with card notices. No interpolated or invented departure times are shown.

Provider metadata adds `realtime_configured`, `active_source` and optional `schedule`; schedule cards also include `schedule`. It contains import/version information, calendar bounds and `valid_until` including actual overnight service. Events use `scheduled_at`, `time_basis: schedule`, `expected_at: null` and never `approaching`. A returned schedule window becomes stale after five minutes and expires after thirty minutes (bounded by feed validity); this is independent of when the GTFS file was downloaded. Refreshes do not extend the feed's coverage. No departures in the query window produces an explicitly scheduled empty state, not a claim that service is canceled.

## Safety and operations

Realtime upstream HTTP uses one reusable client with HTTPS host allowlists, a five-second connect timeout, 12-second total deadline, bounded response bodies, and no redirects. Discovery URLs are additionally limited to Chicago GBFS paths. Errors never expose upstream URLs, keys or response bodies. 429/HTTP `Retry-After` and exponential backoff slow shared collectors. Request bodies are bounded to 16 KiB. Board responses use `Cache-Control: no-store`; no board configurations, addresses or individual-vehicle history are written to disk. Startup and refresh logs contain the bind address and public schedule status, never credentials or provider payloads. Handle deployment access logs with the same privacy policy.
...
The tests use synthetic provider-shaped fixtures, never production vehicle-location archives, and make no provider network calls. They exercise DST/midnight conversion, source age and expiry, schedule/fault/delay flags, mixed/unknown station counts, vehicle eligibility, full-snapshot replacement, partial outages, bounded shared demand, nearest ranking, API bounds and secret-safe failures. Schedule tests additionally cover calendar exceptions, previous-day service, GTFS DST semantics, platform joins, pickup restrictions, frequency handling, missing times, coverage expiry, realtime precedence, first-run loading, hot replacement, corrupt-cache recovery, and retention after download/validation/write failures. Realtime GTFS cancellation joins are not implemented.
